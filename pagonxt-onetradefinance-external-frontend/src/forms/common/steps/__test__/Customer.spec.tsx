...
describe('Form Step Customer', () => {
  ...
  it('displays Name input with error when it is empty and it has been validated', () => {
    renderWithProps();
    const inputClient = getInputByPlaceholder('T_clientName');
  ...
  it('clears the search form when Clear button is clicked', () => {
    renderWithProps();
    ...
    userEvent.click(screen.getByText('T_clean'));
    expect(screen.queryByDisplayValue('Client name')).not.toBeInTheDocument();
  });

  it('does not perform a search if customer name is not fulfilled', () => {
    renderWithProps();
  ...
  it('searches for customers with correct values from the input fields', async () => {
    ...
    changeInputValue(
      getInputByPlaceholder('T_taxId T_fieldOptional')!.querySelector('input')!,
      'TAXID',
    );
    changeInputValue(
  ...
  it('the Continue button appears only when a customer is selected', async () => {
    ...
    userEvent.click(screen.getAllByTestId('radio-button')[0]);
  ...
  it('calls the provided onDataChange method when customer is selected', async () => {
    ...
    expect(defaultProps.onDataChange).toBeCalledWith(customerMockedResponse[0]);
  });
