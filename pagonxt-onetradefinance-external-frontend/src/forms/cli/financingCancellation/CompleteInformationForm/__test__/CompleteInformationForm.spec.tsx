...
import { get as completeInfoResponse } from '../../../../../testUtils/mocks/data/cli/financingCancellation/completeInformation';
...
describe('Form CLI FINANCING CANCELLATION CompleteInformationForm', () => {
  const taskId = 'TSK-123456789';
  const getUrl = `${ApiUrls.cli.financingCancellation.completeInformation.get(
    taskId,
    true,
  )}`;
  ...
  it('fetches and loads the data of the request successfully', async () => {
    ...
    expect(
      screen.getByText(
        formatDate(new Date(request.financingRequest.expirationDate)),
      ),
    ).toBeInTheDocument();
