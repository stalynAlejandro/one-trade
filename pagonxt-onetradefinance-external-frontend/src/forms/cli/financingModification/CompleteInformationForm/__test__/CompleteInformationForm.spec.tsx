...
import { get as completeInfoResponse } from '../../../../../testUtils/mocks/data/cli/financingModification/completeInformation';
...
describe('Form CLI FINANCING MODIFICATION CompleteInformationForm', () => {
  const taskId = 'TSK-123456789';
  const getUrl = `${ApiUrls.cli.financingModification.completeInformation.get(
    taskId,
    true,
  )}`;
  ...
  beforeEach(() => {
    ...
    fetchMockJest.mock(
      ApiUrls.cli.financingModification.completeInformation.complete(taskId),
      {
        body: {
  ...
  it('fetches and loads the data of the request successfully', async () => {
    await renderForm();
    const { request } = completeInfoResponse.entity.request;

    expect(fetchMockJest.lastCall(getUrl)).toBeTruthy();
    expect(screen.getByText(request.financingRequest.code)).toBeInTheDocument();
    expect(
      screen.getByText(
        `${formatNumber(request.financingRequest.amount)} ${
          request.financingRequest.currency
        }`,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        formatDate(new Date(request.financingRequest.creationDate)),
      ),
    ).toBeInTheDocument();
