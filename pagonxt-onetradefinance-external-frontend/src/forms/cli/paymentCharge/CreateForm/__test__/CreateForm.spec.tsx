...
import { formData } from '../../../../../testUtils/mocks/data/cli/paymentCharge/formData';
import { get as submitResponse } from '../../../../../testUtils/mocks/data/cli/paymentCharge/completeInformation';
...
describe('Form CLI PAYMENT CHARGE CreateForm', () => {
  ...
  beforeEach(() => {
    ...
    fetchMockJest.mock(
      ApiUrls.cli.paymentCharge.create.confirm,
      {
  ...
  describe('Draft data operations', () => {
    ...
    const url = ApiUrls.cli.paymentCharge.create.get(collectionRef);
    ...
    it('updates the files IDs, request code and other data with updated draft save response data', async () => {
      ...
      await waitFor(() =>
        expect(
          fetchMockJest.lastCall(ApiUrls.cli.paymentCharge.create.saveDraft),
        ).toBeTruthy(),
      );
      ...
      const requestBody = JSON.parse(
        fetchMockJest.lastCall(ApiUrls.cli.paymentCharge.create.saveDraft)?.[1]
          ?.body as string,
      );
