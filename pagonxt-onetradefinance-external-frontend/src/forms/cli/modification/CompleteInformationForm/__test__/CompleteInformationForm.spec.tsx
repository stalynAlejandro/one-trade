...
import { get as completeInfoResponse } from '../../../../../testUtils/mocks/data/cli/modification/completeInformation';
...
describe('Form CLI MODIFICATION CompleteInformationForm', () => {
  const taskId = 'TSK-123456789';
  const getUrl = `${ApiUrls.cli.modification.completeInformation.get(
    taskId,
    true,
  )}`;
  ...
  beforeEach(() => {
    ...
    fetchMockJest.mock(
      ApiUrls.cli.modification.completeInformation.complete(taskId),
      {
        body: {
  ...
  it('fetches and loads the data of the request successfully', async () => {
    ...
    expect(screen.getByText(request.importCollection.code)).toBeInTheDocument();
    expect(
      screen.getByText(
        `${formatNumber(request.importCollection.amount)} ${
          request.importCollection.currency
        }`,
      ),
    ).toBeInTheDocument();
    expect(
