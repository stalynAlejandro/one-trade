...
import { formData } from '../../../../../testUtils/mocks/data/cli/request/formData';
import { get as submitResponse } from '../../../../../testUtils/mocks/data/cli/request/completeInformation';
...
describe('Form CLI REQUEST CreateForm', () => {
  ...
  beforeEach(() => {
    ...
    fetchMockJest.mock(
      ApiUrls.cli.request.create.confirm,
      {
    ...
    fetchMockJest.mock(
      ApiUrls.cli.request.create.saveDraft,
      {
        body: { ...submitResponse, entity: submitResponse.entity.request },
        status: 200,
      },
  ...
  describe('Operation notifications', () => {
    ...
    it('displays a success message when the Save draft operation succeeds and returns to New Requests page', async () => {
      ...
      expect(await screen.findByText('T_title')).toBeInTheDocument();
      expect(
        await screen.findByText('T_notifications.draftSavedSuccessfully'),
      ).toBeInTheDocument();
  ...
  describe('Draft data operations', () => {
    const collectionRef = 'CLI-123';
    const url = ApiUrls.cli.request.create.get(collectionRef);
    ...
    it('updates the files IDs, request code and other data with updated draft save response data', async () => {
      ...
      await waitFor(() =>
        expect(
          fetchMockJest.lastCall(ApiUrls.cli.request.create.saveDraft),
        ).toBeTruthy(),
      );
      ...
      const requestBody = JSON.parse(
        fetchMockJest.lastCall(ApiUrls.cli.request.create.saveDraft)?.[1]
          ?.body as string,
      );
