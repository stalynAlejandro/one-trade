...
import { get as completeInfoResponse } from '../../../../../testUtils/mocks/data/cle/advanceCancellation/completeInformation';
...
describe('Form CLE ADVANCE CANCELLATION CompleteInformationForm', () => {
  const taskId = 'TSK-123456789';
  const getUrl = `${ApiUrls.cle.advanceCancellation.completeInformation.get(
    taskId,
    true,
  )}`;
  ...
  beforeEach(() => {
    ...
    fetchMockJest.mock(
      ApiUrls.cle.advanceCancellation.completeInformation.complete(taskId),
      {
        body: {
  ...
  it('fetches and loads the data of the request successfully', async () => {
    ...
    const {
      request: { request },
    } = completeInfoResponse.entity;

    expect(fetchMockJest.lastCall(getUrl)).toBeTruthy();
    expect(
      screen.getByText(request.exportCollectionAdvance.code),
    ).toBeInTheDocument();
    expect(
      screen.getByText(request.exportCollectionAdvance.contractReference),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        `${formatNumber(request.exportCollectionAdvance.amount)} ${
          request.exportCollectionAdvance.currency
        }`,
      ),
    ).toBeInTheDocument();
    expect(
    ...
    expect(screen.getByText(request.office)).toBeInTheDocument();
    expect(screen.getByText(request.comments)).toBeInTheDocument();
  });
  ...
  describe('Operation notifications', () => {
    it('displays a success message when the Submit operation succeeds and returns to New Requests page', async () => {
      ...
      expect(
        await screen.findByText('T_successNotificationTitle'),
      ).toBeInTheDocument();
