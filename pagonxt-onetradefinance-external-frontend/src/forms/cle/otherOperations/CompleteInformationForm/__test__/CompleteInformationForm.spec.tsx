...
import { get as completeInfoResponse } from '../../../../../testUtils/mocks/data/cle/otherOperations/completeInformation';
...
describe('Form CLE OTHER OPERATIONS CompleteInformationForm', () => {
  const taskId = 'TSK-123456789';
  const getUrl = `${ApiUrls.cle.otherOperations.completeInformation.get(
    taskId,
    true,
  )}`;
  ...
  it('fetches and loads the data of the request successfully', async () => {
    ...
    expect(
      screen.getByText(`T_operationTypes.${request.request.operationType}`),
    ).toBeInTheDocument();
    expect(
      screen.getByText(request.request.exportCollection.code),
    ).toBeInTheDocument();
    expect(
      screen.getByText(request.request.exportCollection.contractReference),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        `${formatNumber(request.request.exportCollection.amount)} ${
          request.request.exportCollection.currency
        }`,
      ),
    ).toBeInTheDocument();
  });
