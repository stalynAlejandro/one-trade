...
import { get as submitResponse } from '../../../../../testUtils/mocks/data/cle/advance/completeInformation';
...
describe('Form CLE ADVANCE CreateForm', () => {
  ...
  beforeEach(() => {
    ...
    fetchMockJest.mock(
      ApiUrls.cle.advance.create.confirm,
      {
    ...
    fetchMockJest.mock(
      ApiUrls.cle.advance.create.saveDraft,
      {
        body: { ...submitResponse, entity: submitResponse.entity.request },
        status: 200,
      },
  ...
  it('renders the form successfully with first step by default', async () => {
    ...
    expect(
      screen.getByText('T_T_collectionExportationAdvanceAdmission'),
    ).toBeInTheDocument();
  ...
  describe('Operation notifications', () => {
    it('displays a success message when the Submit operation succeeds and returns to New Requests page', async () => {
      ...
      expect(
        await screen.findByText('T_collectionExportationAdvanceAdmission'),
      ).toBeInTheDocument();
      expect(
  ...
  describe('Draft data operations', () => {
    const advanceCollectionRef = 'CLE-ADV-123';
    const url = ApiUrls.cle.advance.create.get(advanceCollectionRef);

    beforeEach(() => {
      mockedUseParams.mockReturnValue({
        advanceCollectionRef,
      });
    ...
    it('updates the files IDs, request code and other data with updated draft save response data', async () => {
      ...
      await waitFor(() =>
        expect(
          fetchMockJest.lastCall(ApiUrls.cle.advance.create.saveDraft),
        ).toBeTruthy(),
      );
      ...
      const requestBody = JSON.parse(
        fetchMockJest.lastCall(ApiUrls.cle.advance.create.saveDraft)?.[1]
          ?.body as string,
      );
    ...
    it('loads the draft data successfully', async () => {
      ...
      // savedStep from mock is 2, so it is Documentation step
      expect(
