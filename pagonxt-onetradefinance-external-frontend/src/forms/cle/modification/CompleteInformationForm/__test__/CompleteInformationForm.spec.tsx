...
import { get as completeInfoResponse } from '../../../../../testUtils/mocks/data/cle/modification/completeInformation';
...
describe('Form CLE MODIFICATION CompleteInformationForm', () => {
  const taskId = 'TSK-123456789';
  const getUrl = `${ApiUrls.cle.modification.completeInformation.get(
    taskId,
    true,
  )}`;
  ...
  it('fetches and loads the data of the request successfully', async () => {
    ...
    expect(screen.getByText(request.exportCollection.code)).toBeInTheDocument();
    expect(
      screen.getByText(
        `${formatNumber(request.exportCollection.amount)} ${
          request.exportCollection.currency
        }`,
      ),
    ).toBeInTheDocument();
    expect(
