...
import { get as submitResponse } from '../../../../../testUtils/mocks/data/cle/request/completeInformation';
...
describe('Form CLE REQUEST CreateForm', () => {
  ...
  beforeEach(() => {
    ...
    fetchMockJest.mock(
      ApiUrls.cle.request.create.confirm,
      {
    ...
    fetchMockJest.mock(
      ApiUrls.cle.request.create.saveDraft,
      {
        body: { ...submitResponse, entity: submitResponse.entity.request },
        status: 200,
      },
  ...
  it('renders the form successfully with first step by default', async () => {
    ...
    expect(
      screen.getByText('T_T_collectionExportationAdmission'),
    ).toBeInTheDocument();
  ...
  it('hides the progress bar in the Confirm step', async () => {
    await renderForm({ initialFormData: formData, initialStep: 4 });
  ...
  describe('Operation notifications', () => {
    it('displays a success message when the Submit operation succeeds and returns to New Requests page', async () => {
      ...
      expect(
        await screen.findByText('T_collectionExportationAdmission'),
      ).toBeInTheDocument();
      expect(
        await screen.findByText('T_collectionCreatedSuccessfully'),
      ).toBeInTheDocument();
    ...
    it('displays a success message when the Save draft operation succeeds and returns to New Requests page', async () => {
      ...
      expect(
        await screen.findByText('T_collectionDraftSavedSuccessfully'),
      ).toBeInTheDocument();
  ...
  describe('Draft data operations', () => {
    const collectionRef = 'CLE-123';
    const url = ApiUrls.cle.request.create.get(collectionRef);
    ...
    it('updates the files IDs, request code and other data with updated draft save response data', async () => {
      ...
      await waitFor(() =>
        expect(
          fetchMockJest.lastCall(ApiUrls.cle.request.create.saveDraft),
        ).toBeTruthy(),
      );
      ...
      const requestBody = JSON.parse(
        fetchMockJest.lastCall(ApiUrls.cle.request.create.saveDraft)?.[1]
          ?.body as string,
      );
