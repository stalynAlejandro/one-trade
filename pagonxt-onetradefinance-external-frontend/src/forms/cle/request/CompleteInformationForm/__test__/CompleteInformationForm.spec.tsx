...
import { get as completeInfoResponse } from '../../../../../testUtils/mocks/data/cle/request/completeInformation';
...
describe('Form CLE REQUEST CompleteInformationForm', () => {
  ...
  const getUrl = `${ApiUrls.cle.request.completeInformation.get(taskId, true)}`;
  ...
  it('renders the form successfully with last step by default', async () => {
    ...
    expect(
      screen.getByText('T_T_completeCollectionAdmissionInformation'),
    ).toBeInTheDocument();
  ...
  it('fetches and loads the data of the request successfully', async () => {
    ...
    expect(
      screen.getByText(
        completeInfoResponse.entity.request.operationDetails.clientAccount.iban,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        completeInfoResponse.entity.request.operationDetails.commissionAccount
          .iban,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        `${formatNumber(
          completeInfoResponse.entity.request.operationDetails.collectionAmount,
        )} ${
          completeInfoResponse.entity.request.operationDetails
    ...
    expect(
      screen.getByText(
        `${formatNumber(
          completeInfoResponse.entity.request.advance.advanceAmount,
        )} ${
          completeInfoResponse.entity.request.advance.advanceCurrency.currency
        }`,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        completeInfoResponse.entity.request.advance.riskLine.iban,
      ),
    ).toBeInTheDocument();
  });
  ...
  it('changes the step successfully and renders the corresponding step', async () => {
    ...
    expect(
      screen.getByText('T_indicateAdvanceDetails', {
        selector: '.stepTitle__title',
      }),
    ).toBeInTheDocument();
  });
  ...
  it('shows a confirmation message when the Close button is pressed', async () => {
    ...
    userEvent.click(screen.getByText('T_discard'));
  ...
  it('navigates to Pending Tasks page when confirming Close dialog', async () => {
    ...
    userEvent.click(getByText('T_confirm'));

    expect(mockedUsedNavigate).lastCalledWith(
  ...
  describe('Operation notifications', () => {
    it('displays a success message when the Submit operation succeeds and returns to New Requests page', async () => {
      ...
      expect(
        await screen.findByText('T_completeCollectionAdmissionInformation'),
      ).toBeInTheDocument();
      expect(
        await screen.findByText('T_informationSubmittedSuccessfully'),
      ).toBeInTheDocument();
