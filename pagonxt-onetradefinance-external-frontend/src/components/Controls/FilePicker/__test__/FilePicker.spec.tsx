...
describe('Control FilePicker', () => {
  ...
  const uploadFilesToInput = async (files: File[] = []) => {
    ...
    await waitFor(() => {
      expect(defaultProps.onLoad).toBeCalledTimes(1);
    });
  ...
  it('file input is triggered when clicking to the button', () => {
    renderWithProps();
  ...
  it('file input is triggered when clicking anywhere in the dropzone', () => {
    renderWithProps();
    const container = screen.getByTestId('file-picker');
    const input = screen.getByTestId('file-picker-input');
