...
class UserStdSerializerTest {
    ...
    private final UserStdSerializer userStdSerializer = new UserStdSerializer();
    ...
    void whenSerializingDocument_ok_returnsValidData() throws IOException {
        ...
        User user = mapper.readValue(new ClassPathResource("data/model/user.json").getFile(), User.class);
        JsonNode expectedJsonNode = mapper.readTree(new ClassPathResource("data/model/serialized-user.json").getFile());

        // When
        userStdSerializer.serialize(user, jsonGenerator, mapper.getSerializerProvider());
        jsonGenerator.flush();
